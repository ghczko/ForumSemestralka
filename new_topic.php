...
<div class="loginmodal-container">
    ...
    <form action="" method="POST">
        ...
        <div class="form-group">
            <label for="comment">Kategorie tématu</label>
            <?php if (isset($picked_cat)) { ?>
                <select name="topic_cat" id="disabledInput" class='selectpicker' disabled>
            ...
            <?php } else { ?>
                <select name="topic_cat" class='selectpicker'>
                    <?php foreach ($cat as $row) { ?>
                        <option value="<?= $row['cat_id'] ?>"><?= $row['cat_name'] ?> </option>
                    <?php } ?>
                </select>
        ...
        <div class="form-group">
            <label for="comment">Tagy</label><br/>
            <select name=topic_tags[] class='selectpicker' multiple>
                <?php foreach ($tags as $row) { ?>
                    <option value="<?= $row['id'] ?>"><span class='label label-success'><?= $row['tags_name'] ?> </span>
                    </option>
                <?php } ?>
            </select>
        </div>
